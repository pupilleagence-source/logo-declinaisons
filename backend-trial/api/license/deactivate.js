/**
 * API Endpoint: /api/license/deactivate
 * Désactive la licence d'un poste : libère l'instance Lemon Squeezy, puis Redis.
 *
 * POST Body: { licenseKey: string, hwid: string }
 * Response: { success: boolean, message: string, deleted: boolean }
 */

import { getRedisClient, preamble } from '../../lib/redis.js';
import { createLemonClient } from '../../lib/lemonsqueezy.js';
import { deactivateLicense } from '../../lib/license-flow.js';

export default async function handler(req, res) {
    if (preamble(req, res, ['POST'])) return;
    try {
        const store = await getRedisClient();
        const result = await deactivateLicense(req.body, { ls: createLemonClient(), store });
        return res.status(result.status).json(result.body);
    } catch (error) {
        console.error('Erreur /api/license/deactivate:', error);
        return res.status(500).json({ success: false, message: 'Erreur serveur lors de la désactivation' });
    }
}
