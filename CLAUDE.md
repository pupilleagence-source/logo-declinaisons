...
**Mode test / mode réel.** La boutique n'est jamais passée en réel (constaté le 2026-09-08). Produits et variantes sont communs aux deux modes ; commandes, clients, clés de licence, clés API et webhooks sont séparés. Pour vendre : activer la boutique (Settings → General), couper le mode test, **recréer clé API + webhook en mode réel**, les mettre dans Vercel (`vercel env rm/add … production` depuis un fichier dans `apple-cert/`, jamais dans le chat), redéployer. Test sans payer : achat en mode test avec la carte `4242 4242 4242 4242`.
