...
- **Release `v1.3.0` publiée le 2026-09-06 (19 h 02)** via `npm run release` — première utilisation du script, de bout en bout (bump, tests, commit, push, workflow, notarisation `Accepted`). Contient : trial à 3, bouton Annuler, popup macOS retirée, et tous les correctifs de relecture. **Backend à déployer** (`vercel --prod`) pour que le trial à 3 soit effectif côté serveur et que les installs 1.1.0 / 1.2.0 voient la modale.
